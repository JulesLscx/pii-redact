...
The core is a dependency-free Python package. Integration modes:
...
**4. Telemetry** — observe pseudonymization without intercepting it. The connector emits one record per core call carrying only counters, sizes and timings — no field can hold document text, a detected value, or a token (`EVENT_FIELDS` is a closed set, enforced by `is_safe_record()`).
...
safe = probe.redact(document).text                   # → LLM, plus one record
answer = probe.restore(llm_output)                   # → back, plus one record
probe.summary()  # {'calls': 42, 'spans': 517, 'counts': {...}, 'p95_ms': 11.2}
```

Three sinks ship the records (`JsonLinesSink`, `MemorySink`, `HttpSink`), each fail-safe — a down collector increments `dropped` and stays invisible. If a host already calls the core elsewhere (Hermes hook, stdio server), inject the result instead of wrapping: `probe.observe(result, source="hook")`.

### Adapters

| Module | Host / protocol | Direction |
|---|---|---|
| `piiredact/adapters/hermes.py` | in-process Hermes plugin (hooks + middleware) | outbound + inbound |
| `piiredact/adapters/stdio.py` | line-delimited JSON on stdin/stdout (OpenCode, Codex, editors, scripts) | outbound + inbound |
| `piiredact/adapters/claude_code.py` | Claude Code hooks (`PreToolUse` / `PostToolUse`) | outbound + inbound |
| `piiredact/adapters/telemetry.py` | observability: JSON lines, memory, batched `POST` HTTP | outbound (measure) |

To write another one: [docs/connectors.md](docs/connectors.md) — minimal contract, decision tree, pitfalls (restoration on critical path, block mode, prompt caching) and checklist.
...
See [docs/SPEC.md](docs/SPEC.md) for invariants, threat model, and design trade-offs, and [docs/connectors.md](docs/connectors.md) for how to wire in another host.
